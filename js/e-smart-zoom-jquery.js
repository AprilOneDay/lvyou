  ...
  $.fn.smartZoom = function(method) {
    ...
    var publicMethods = {
	    ...
	    init : function(options) {
		    ...
		    var zoomContainerId = "smartZoomContainer";
		  ...
		  //   computeSelf($("#imagesMap"))
	 		// computeSelf($("#imageFullScreen"))
			//computeSelf($("#smartZoomContainer"))
		    ...
		    targetElement.data('smartZoomData', {
               ...
               originalSize: {width:targetElement.width(), height:targetElement.height()}, // plugin target size at the beginning //originalSize: {width:originalSizeComputed.x, height:originalSizeComputed.y}, 
            ...
            //console.log(targetElement.data("smartZoomData"))
	   ...
	   /**
	    ...
	    zoom : function(scaleToAdd, globalRequestedPosition, duration){
			...
			// console.log(positionGlobalXDiff,"positionGlobalXDiff")
    ...
    function touchMoveHandler(e){ 
		...
		if(nbTouch == 1 && !smartData.touch.touchPinch && smartData.settings.touchMoveEnabled == true){ // if the user use only one finger and touchPinch==false => we manage drag
			...
			// console.log(currentFirstTouchEv)
			...
			if(Math.abs(currentDistance)<1.8) // finger pixel error (jump between in and out mode sometimes if i don't do this check)  
				return;
    ...
    function moveOnMotion(xPos, yPos, duration, motionEnd){
		...
		//console.log(targetElement, validPosition.x, validPosition.y, targetRect.width, targetRect.height, duration, motionEnd)
		...
		// console.log(smartData.moveCurrentPosition)
	}
    ...
    function getValidTargetElementPosition(xPosition, yPosition, width, height){
	    ...
	    var newMarginTop = Math.min(0, yPosition); // adjust in top 

	    newMarginTop += Math.max(0, (smartData.adjustedPosInfos.top*2 + smartData.adjustedPosInfos.height) - (newMarginTop + height)) // adjust in bottom
	    var newMarginLeft = Math.min(0, xPosition); // adjust in left
	    //console.log(smartData.adjustedPosInfos.left,xPosition)
	    //console.log(smartData.adjustedPosInfos.left,xPosition)
	    newMarginLeft += Math.max(0, (smartData.adjustedPosInfos.left*2 + smartData.adjustedPosInfos.width) - (newMarginLeft + width)); // adjust in right
	    //console.log(newMarginTop,newMarginLeft)
	     //console.log(typeof $("#imageFullScreen").css("transform"))
	     adjustPointerPos()
    ...
    function animate(target, globalLeft, globalTop, width, height, duration, callback){
    	...
    	if (smartData.transitionObject != null) { // use css transition if supported
    		var originalSize = smartData.originalSize;
		  	...
		  	cssObject[smartData.transitionObject.transition] = 'all '+duration / 3000+'s ease-out'; // set effect duration
...
// 	if(flag){
// 		$(_this).width(size.x)
// 		$(_this).height(size.Cy)
// 	}else{
// 		$(_this).width(size.y)
// 		$(_this).height(size.Cx)
